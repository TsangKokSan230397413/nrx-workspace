...
function encodeShareData() {
  const payload = {
    ...
    g: getGoalText(),
    items: prescriptionItems().map(item => ({ id: item.id, reps: item.reps, sets: item.sets }))
  };
...
function loadFromShareParamIfPresent() {
  const rxParam = new URLSearchParams(window.location.search).get("rx");
  if (!rxParam) return false;
  try {
    const data = decodeShareData(rxParam);
    enterPatientView(data);
    return true;
  } catch (error) {
...
const isPatientView = loadFromShareParamIfPresent();
if (!isPatientView) restoreDraftFromStorage();
