...
// 依照你的要求，內建一組預設金鑰，使用者不需要自己申請即可使用。
// 這是你自己在 Google AI Studio 申請的免費金鑰，請務必到 Google Cloud Console
// 幫這組金鑰加上「HTTP 網域限制」，鎖定只能從你的網站網域呼叫（詳細說明在對話裡）。
...
function shareViaLine() {
  ...
  // 電腦版瀏覽器沒辦法像手機一樣，用一般網址自動叫出LINE桌面版程式，
  // 所以這裡先「嘗試」用 line:// 這個專屬通道直接打開LINE桌面版；
  // 如果電腦有裝LINE，通常會跳出瀏覽器詢問「是否要打開LINE」的確認視窗。
  // 如果 1.2 秒後畫面還在（代表沒有LINE可以接手，或使用者按了取消），
  // 就自動改用網頁版 line.me 當備案，手機瀏覽器打開網頁版時大多會直接跳轉進LINE App。
  let handedOff = false;
...
function getAiKey() {
  return localStorage.getItem(AI_KEY_STORAGE) || DEFAULT_AI_KEY;
}
...
async function callGeminiApi(userText, { structured = false } = {}) {
  ...
  for (const model of modelsToTry) {
    response = await callGeminiOnce(model, apiKey, body);
    if (response.ok) {
...
function handleAiError(error) {
  if (error.code === "NO_KEY") {
    appendAiMessage("error", "請先在上方「AI 金鑰設定」貼上你的 Gemini API 金鑰（免費申請）。");
  ...
  if (/API key not valid|API_KEY_INVALID|400/i.test(msg)) {
    appendAiMessage(
      "error",
      `發生錯誤：${msg}\n\n這通常代表金鑰格式不對。請到 https://aistudio.google.com/apikey 重新確認，正確的 Gemini API 金鑰應該是 "AIzaSy" 開頭的一長串英數字，不是其他格式的憑證，複製後貼到上方「AI 金鑰設定」重新儲存。`
    );
    const settings = document.querySelector("#aiKeySettings");
    if (settings) settings.open = true;
    return;
...
if (saveAiKeyBtn) {
  saveAiKeyBtn.addEventListener("click", () => {
    ...
    appendAiMessage("assistant", "金鑰已儲存在這台裝置的瀏覽器裡。");
  });
}
